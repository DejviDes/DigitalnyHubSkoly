...
if (contactForm) {
  ...
  const formStartField = contactForm.querySelector("[data-form-start]");
  ...
  contactForm.addEventListener("submit", async (event) => {
    ...
    const localLeft = getLocalCooldownLeft();
    if (localLeft > 0) {
      const mins = Math.max(1, Math.ceil(localLeft / 60000));
      setStatus(
        `Z tohto zariadenia ste už odoslali žiadosť. Skúste to prosím znova približne o ${mins} min.`,
        "is-error",
      );
      return;
    }
    ...
    if (!turnstileToken) {
      setStatus(
        "Bezpečnostná kontrola ešte neprebehla. Počkajte chvíľu a skúste to znova.",
        "is-error",
      );
      return;
    }
    ...
    const payload = {
      ...
      formStartTs: Number(formData.get("formStartTs") || 0),
      turnstileToken,
    };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorPayload?.code === "CAPTCHA_FAILED") {
          setStatus(
            "Bezpečnostná kontrola zlyhala. Obnovte stránku a skúste znova.",
            "is-error",
          );
          if (window.turnstile) {
            try {
        ...
        if (errorPayload?.code === "DAILY_CAP_REACHED") {
          setStatus(
            "Dnešný limit pre prijímanie žiadostí bol dosiahnutý. Skúste to prosím zajtra alebo nás kontaktujte priamo e-mailom.",
            "is-error",
          );
          return;
        }
        ...
        if (response.status === 429 || errorPayload?.code === "RATE_LIMITED") {
          ...
          setStatus(
            `Žiadosť bola dočasne obmedzená kvôli prevencii spamu.${retryMessage}`,
            "is-error",
          );
          return;
        }
